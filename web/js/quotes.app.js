...
myApp.config(['$routeProvider', function ($routeProvider) {
    ...
    .controller('HomeCtrl', ['$scope', '$http', 'ngProgress', '$location', 'ctgId', '$window',
        function ($scope, $http, ngProgress, $location, ctgId, $window) {
            ...
            ngProgress.start();
            var promise = $http.get('/userDetails/QuotesApp').success(function (data, status, headers, config) {
                angular.element(document.querySelector('title')).text('Welcome! ' + data.displayName);
                ctgId.hasWriteAccess = data.permissions[1];
